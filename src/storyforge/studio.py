...
No cloud dependency is required. The server binds to localhost by default and edits the
canonical StoryForge project files under STORYFORGE_PROJECTS_DIR.
"""
...
def list_projects():
    ...
    for p in sorted(_projects_root().iterdir()):
        if not p.is_dir():
            continue
        has_manifest = (p / "work" / "manifest.json").exists()
        result.append({"name": p.name, "has_manifest": has_manifest})
    return result
